...
weak = int(strong*.1)
write_force(300,int(expInfo['offset_L']),int(expInfo['offset_R']))
...
ForceTextFort = visual.TextStim(win, 
                        units='norm',height = 0.1, pos=[0, 0],
                        text='Niveaux de force : Forte',color='white',autoLog = False)

ok = visual.TextStim(win, 
                        units='norm',height = 0.1, pos=[0, 0],
                        text='OK',color='green',autoLog = False)

ForceTextFaible = visual.TextStim(win, 
                        units='norm',height = 0.1, pos=[0, 0],
                        text='Niveaux de force : Faible',color='white',autoLog = False)
...
ForceFortPending = True
ForceFaiblePending = False
continueRoutine=True
while continueRoutine:
    if ForceFortPending == True:
        continueForceRoutine = True
        write_force(strong, int(expInfo['offset_L']),int(expInfo['offset_R']))
        ForceTextFort.draw()
        win.flip()
        core.wait(5)
        while continueForceRoutine:
            ForceTextFort.draw()
            win.flip()
            lecture_port = ctypes.windll.inpout32.Inp32(0x379)
            if lecture_port  in port_gauche or lecture_port  in port_droit:
                ...
                core.wait(3)
            if event.getKeys(["space"]):#seul l'experimentateur peut passer cet écran
                ForceFaiblePending = True
                ForceFortPending = False
                continueForceRoutine = False
    if ForceFaiblePending == True:
        continueForceRoutine = True
        write_force(weak, int(expInfo['offset_L']),int(expInfo['offset_R']))
        ForceTextFaible.draw()
        win.flip()
        core.wait(5)
        while continueForceRoutine:
            ForceTextFaible.draw()
