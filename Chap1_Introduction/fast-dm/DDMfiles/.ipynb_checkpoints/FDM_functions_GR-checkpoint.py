...
Inspired by https://github.com/Gilles86/fastdm
"""
...
custom_dict = {'a':0, 'zr':1, 'v':2,'t0':3,'szr':4,'sv':5,'st0':6, 'd':7}#order parameters
color = {'1':'r', '2':'b','3':'g', '4':'y', '5':'orange'}
SAT = ['Accuracy','Speed']
contraste = ['1','2','3']
...
def generate_files(dataset, free = {}, fixed = {}, method=None, precision=3):
    cols = ['RT','response','contraste','SAT','givenResp']
...
#==============================================================================
        ...
        cfg_file.write('format TIME RESPONSE contraste SAT givenResp\n')
...
#Writing data file
#==============================================================================
        temp = dataset[dataset.participant == x]
...
def get_parameters():
    ...
    for f in list_dir:
        ...
        if i == "parameter":#only file with parameter* taken
            par = pd.read_table(f,header=0,delim_whitespace=True)
            par = par.T.reset_index()
            participant = par[0][0].split('_')[1:]
            fit = par[0].iloc[-3]
            par = par[1:-4]
            par = par.reset_index()
            par.loc[par['index'].str.contains('Accuracy'), 'SAT'] = 'Accuracy'
            par.loc[par['index'].str.contains('Speed'), 'SAT'] = 'Speed'
            par.loc[par['index'].str.contains('Right'), 'givenResp'] = 'Right'
            par.loc[par['index'].str.contains('Left'), 'givenResp'] = 'Left'
            par.loc[par['index'].str.contains('0.01'), 'contraste'] = 0.01
            par.loc[par['index'].str.contains('0.025'), 'contraste'] = 0.025
            par.loc[par['index'].str.contains('0.07'), 'contraste'] = 0.07
            par.loc[par['index'].str.contains('0.15'), 'contraste'] = 0.15
            par.loc[par['index'].str.contains('0.3'), 'contraste'] = 0.3
            parameter = []
            ...
            par['participant'] = '_'.join(participant)
            par['fit'] = fit
            list_.append(par)
