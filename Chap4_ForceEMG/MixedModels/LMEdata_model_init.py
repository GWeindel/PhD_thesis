...
LMEdata = data[data.response == 1].copy() #Correct response only
LMEdata["logrt"] = np.log(LMEdata.rt)
LMEdata["logprmt"] = np.log(LMEdata.prmt)
LMEdata["logmt"] = np.log(LMEdata.mt)
LMEdata['FC'] = LMEdata.apply(lambda row: -0.5 if row['FC'] == 1 else 0.5, axis=1)
print(LMEdata.FC.unique())
LMEdata['contrast'] = LMEdata.contrast.replace([1,2,3,4,5,6], [-.5,-.3,-.1,.1,.3,.5])
LMEdata['SAT'] = LMEdata.apply(lambda row: 0 if row['SAT'] == 'Accuracy' else 1, axis=1)
LMEdata['participant'] = [int(x.split('S')[-1]) for x in LMEdata.participant]

LMEdata = LMEdata[["logrt","logprmt","logmt","participant",
                   "SAT","contrast","FC"]]
