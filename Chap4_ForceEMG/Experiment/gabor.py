...
"S4":c4,"S8":c4,"S12":c4,"S16":c4,"S20":c4}



delta = .07 #Contrast difference between Left and right
NtrialsSession = 2400 #number of trials per session
alternate = NtrialsSession/8 #number of trials per successive condition
changeForce = NtrialsSession/4 #number of trials before changing force level
...
txtChangeForce =visual.TextStim(win, ori=0,height = 0.5, name='pause',
    text=u"Changement de force")
...
#############################TRIALS
#Trial Handler
trials=data.TrialHandler(nReps=NtrialsSession/alternate, method=u'random',
...
#                        FBPending = False
            if pausePending:
                ...
                if counter % changeForce == 0 and boolChangeForce == True:#Manually change force level + trigger
                    txtChangeForce.draw()
                    win.flip()
                    if force == "weak":
                        force = "strong"
